...
- (void)drawRect:(CGRect)rect {
    ...
    //添加九个宫格视图
    double gridGap = (SCREEN_WIDTH - GridWidth * 3) / 4;
    double startY = SCREEN_HEIGHT - gridGap * 2 - GridWidth * 3 - GridWidth - 10;
    for (NSInteger n=0; n<3; n++) {
        for (NSInteger i=0; i<3; i++) {
            double gridX = gridGap * (i + 1) + (GridWidth * i);
            NGGridView *gridView = [[NGGridView alloc] initWithFrame:CGRectMake(gridX, startY + GridWidth * n + gridGap * n, GridWidth + 2, GridWidth + 2)];
